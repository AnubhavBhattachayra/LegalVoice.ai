...
export async function GET(
  ...
  try {
    ...
    // Build query
    const query: any = { lawyerId: new ObjectId(id) };
    ...
    return NextResponse.json(
      { error: 'Failed to fetch lawyer bookings' },
      { status: 500 }
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    const { id } = params;
    const { date, time, duration, topic } = await req.json();
...
export async function PUT(
  ...
  try {
    ...
    if (status === 'cancelled') {
      await db.collection('users').updateOne(
        { _id: session.user.id },
        { $inc: { credits: existingBooking.cost } }
      );

      // Create refund transaction
      await db.collection('creditTransactions').insertOne({
        userId: session.user.id,
        amount: existingBooking.cost,
        type: 'refund',
        description: `Refund for cancelled consultation with ${existingBooking.lawyer.name}`,
...
export async function DELETE(
  ...
  try {
    ...
    await db.collection('creditTransactions').insertOne({
      ...
      amount: existingBooking.cost,
      type: 'refund',
      description: `Refund for deleted consultation with ${existingBooking.lawyer.name}`,
      bookingId: new ObjectId(bookingId),
      createdAt: new Date()
    });
